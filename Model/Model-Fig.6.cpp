...
int Neumann1=31; //Force-added boundary
int Neumann2=32; //Force-added boundary

//Cell shape aspect ratio
real asp=1.0;

border a11(t=0,0.0001){x=asp*cos(pi/6)*t;y=sin(pi/6)*t;label=Neumann1;} 
border a12(t=0.0001,1){x=asp*cos(pi/6)*t;y=sin(pi/6)*t;label=Neumann1;} 
border a2(t=0,1){x=asp*cos(pi/6);y=t+sin(pi/6);label=Neumann2;} 
...
border a6(t=0,1){x=asp*(cos(pi*7/6));y=1+sin(pi/6)-t;label=Neumann2;} 
border a71(t=0,0.9999){x=asp*(cos(pi*7/6)+cos(pi*11/6)*t);y=sin(pi/6)+sin(11*pi/6)*t;label=Neumann1;} 
border a72(t=0.9999,1){x=asp*(cos(pi*7/6)+cos(pi*11/6)*t);y=sin(pi/6)+sin(11*pi/6)*t;label=Neumann1;} 
...
Vh2 [g1,g2]=[0,-100]; //ï–éùÇøó¿ÇÃí[Ç…ó^Ç¶ÇÈóÕ
...
//	-int1d(Th,Neumann1)(p*N.x*vf1+p*N.y*vf2)
	-int1d(Th,Neumann2)(p*N.x*vf1+p*N.y*vf2)
	-int1d(Th,Neumann)(-p11*vf2)
	-int1d(Th,Neumann1)(p11*vf2)
